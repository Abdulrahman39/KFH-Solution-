...
const Releases = () => {
    ...
    const statuses = ['Done', 'Uploaded', 'Under Testing'];
    ...
    const representativeFilterTemplate = (options) => {
        return (
            <>
                <div className="mb-3 text-bold">Developer Picker</div>
    ...
    const descriptionBodyTemplate = (rowData) => {
        return <span style={{ marginLeft: '.5em', verticalAlign: 'middle' }}>description...</span>
    };
    ...
    const statusBodyTemplate = (rowData) => {
        ...
        return <span className={`customer-badge status-${statuses[randomIndx] == "uploaded" ? "new" : statuses[randomIndx] == "under testing" ? "proposal" : statuses[randomIndx] == "done" ? "qualified" : ""}`}>{statuses[randomIndx]}</span>;
    };
    ...
    let items = [
        ...
        {
            label: 'Releases', icon: 'pi pi-fw pi-tags', className: 'bg-green-100'
